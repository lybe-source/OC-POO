...
class PersonnagesManager
{
  ...
  public function add(Personnage $perso)
  {
    ...
    $q->bindValue(':nom', $perso->nom()); // Permet d'associer la valeur de nom à l'attribut $nom de la classe Personnage
    $q->bindValue(':type', $perso->type());
    
    $q->execute();
    
    $perso->hydrate([
      'id' => $this->db->lastInsertId(), // Retourne l'identifiant de la dernière ligne insérée ou la valeur d'une séquence
      'degats' => 0,
      'atout' => 0
    ]);
  }
  ...
  {
    return $this->db->query('SELECT COUNT(*) FROM personnages_v2')->fetchColumn(); // PDOStatement::fetchColumn — Retourne une colonne depuis la ligne suivante d'un jeu de résultats
  }
  ...
  public function getList($nom)
  {
    ...
    $q = $this->db->prepare('SELECT id, nom, degats, timeEndormi, type, atout FROM personnages_v2 WHERE nom <> :nom ORDER BY nom'); // Sélectionne l'id, les dégats, le temps d'endormissement, le type et l'atout de la table personnages_v2 où nom n'est pas égal à :nom par ordre alphabétique par nom
    $q->execute([':nom' => $nom]);
  ...
  public function update(Personnage $perso)
  {
    ...
    $q->bindValue(':degats', $perso->degats(), PDO::PARAM_INT); // On associe la valeur de :degats à l'attribut $degats de la classe Personnage, PDO::PARAM_INT (int) Représente le type de données INTEGER SQL.
